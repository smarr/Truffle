...
/**
 * Specialization of {@link jdk.graal.compiler.lir.alloc.lsra.LinearScanAssignLocationsPhase} that
...
final class TraceLinearScanAssignLocationsPhase extends TraceLinearScanAllocationPhase {
    ...
    private static final class Assigner {
        ...
        private void assignBlock(BasicBlock<?> block) {
        ...
        private void handleBlockBegin(BasicBlock<?> block, ArrayList<LIRInstruction> instructions) {
        ...
        private void handleBlockEnd(BasicBlock<?> block, ArrayList<LIRInstruction> instructions) {
        ...
        private final InstructionValueProcedure assignProc = new InstructionValueProcedure() {
            ...
            public Value doValue(LIRInstruction instruction, Value value, OperandMode mode, EnumSet<OperandFlag> flags) {
                if (isVariable(value)) {
                    return colorLirOperand(instruction, asVariable(value), mode);
                }
                return value;
            }
        };
        ...
        private void assign() {
            try (Indent indent = allocator.getDebug().logAndIndent("assign locations")) {
                for (BasicBlock<?> block : allocator.sortedBlocks()) {
                    assignBlock(block);
                }
            }
        }
