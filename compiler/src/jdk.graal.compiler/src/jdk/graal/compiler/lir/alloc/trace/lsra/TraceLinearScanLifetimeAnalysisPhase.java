...
import static jdk.graal.compiler.lir.alloc.trace.TraceRegisterAllocationPhase.Options.TraceRAshareSpillInformation;
import static jdk.graal.compiler.lir.alloc.trace.TraceRegisterAllocationPhase.Options.TraceRAuseInterTraceHints;
...
import jdk.graal.compiler.lir.alloc.trace.ShadowedRegisterValue;
import jdk.graal.compiler.lir.alloc.trace.lsra.TraceInterval.RegisterPriority;
import jdk.graal.compiler.lir.alloc.trace.lsra.TraceInterval.SpillState;
...
public final class TraceLinearScanLifetimeAnalysisPhase extends TraceLinearScanAllocationPhase {
    ...
    public static final class Analyser {
        ...
        private BasicBlock<?>[] sortedBlocks() {
        ...
        private void buildIntervals() {
            ...
            try (Indent indent = debug.logAndIndent("build intervals")) {
                ...
                BasicBlock<?>[] blocks = sortedBlocks();
                for (int blockId = blocks.length - 1; blockId >= 0; blockId--) {
                    final BasicBlock<?> block = blocks[blockId];
                    ...
                    try (Indent indent2 = debug.logAndIndent("handle block %d", block.getId())) {
                        ...
                        BasicBlock<?> pred = blockId == 0 ? null : blocks[blockId - 1];
        ...
        private void handleTraceBegin(BasicBlock<?> block) {
        ...
        private void handleBlockBegin(BasicBlock<?> block, BasicBlock<?> pred) {
        ...
        private void handleBlockEnd(BasicBlock<?> block, int opId) {
        ...
        private void addInterTraceHints() {
            try (DebugContext.Scope s = debug.scope("InterTraceHints", allocator)) {
                BasicBlock<?> traceHeadBlock = sortedBlocks()[0];
