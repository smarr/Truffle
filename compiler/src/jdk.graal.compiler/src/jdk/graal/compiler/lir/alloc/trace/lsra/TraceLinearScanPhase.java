...
import jdk.graal.compiler.lir.alloc.trace.TraceUtil;
...
public final class TraceLinearScanPhase extends TraceAllocationPhase<TraceAllocationContext> {
    ...
    private static final Comparator<TraceInterval> SORT_BY_FROM_COMP = new Comparator<>() {
    ...
    private static final Comparator<TraceInterval> SORT_BY_SPILL_POS_COMP = new Comparator<>() {
    ...
    public final class TraceLinearScan implements IntervalDumper {
        ...
        int numLoops() {
            return getLIR().getControlFlowGraph().getNumberOfLoops();
        }
        ...
        private void verifyRegisters() {
            ...
            try (Indent indent = getDebug().logAndIndent("verifying register allocation")) {
                TraceRegisterVerifier verifier = new TraceRegisterVerifier(this);
        ...
        public BasicBlock<?>[] sortedBlocks() {
         ...
         * block} containing the instruction. Entries should be retrieved with
        ...
        boolean hasInterTracePredecessor(BasicBlock<?> block) {
            return TraceUtil.hasInterTracePredecessor(traceBuilderResult, trace, block);
        }

        boolean hasInterTraceSuccessor(BasicBlock<?> block) {
