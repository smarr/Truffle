...
public final class GlobalLivenessInfo {
    ...
    public int[] getBlockOut(BasicBlock<?> block) {
        return blockToVarOut[block.getId()];
    }

    public int[] getBlockIn(BasicBlock<?> block) {
        return blockToVarIn[block.getId()];
    }

    public void setInLocations(BasicBlock<?> block, Value[] values) {
        blockToLocIn[block.getId()] = values;
    }

    public void setOutLocations(BasicBlock<?> block, Value[] values) {
        blockToLocOut[block.getId()] = values;
    }

    public Value[] getInLocation(BasicBlock<?> block) {
        return blockToLocIn[block.getId()];
    }

    public Value[] getOutLocation(BasicBlock<?> block) {
    ...
    private boolean verifyBlock(BasicBlock<?> block, LIR lir) {
