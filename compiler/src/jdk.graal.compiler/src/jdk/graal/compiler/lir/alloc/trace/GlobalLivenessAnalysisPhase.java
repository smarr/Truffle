...
public final class GlobalLivenessAnalysisPhase extends AllocationPhase {
    ...
    private static final class Analyser {
        ...
        private BitSet getLiveIn(final BasicBlock<?> block) {
            return liveIns[block.getId()];
        }

        private BitSet getLiveOut(final BasicBlock<?> block) {
            return liveOuts[block.getId()];
        }

        private void setLiveIn(final BasicBlock<?> block, final BitSet liveIn) {
            liveIns[block.getId()] = liveIn;
        }

        private void setLiveOut(final BasicBlock<?> block, final BitSet liveOut) {
        ...
        private void computeLiveness() {
            ...
            for (int i = blocks.length - 1; i >= 0; i--) {
                final BasicBlock<?> block = blocks[i];
         ...
         */
        private void handleLoopHeader(CFGLoop<?> loop, BitSet live) {
        ...
        public void build() {
            ...
            BasicBlock<?> startBlock = getLIR().getControlFlowGraph().getStartBlock();
        ...
        public void finish() {
            ...
            for (BasicBlock<?> block : (BasicBlock<?>[]) lir.getControlFlowGraph().getBlocks()) {
