 ...
 * questions.
 */
package jdk.graal.compiler.lir.alloc.trace.bu;
...
import jdk.graal.compiler.lir.alloc.trace.TraceGlobalMoveResolutionPhase;
import jdk.graal.compiler.lir.alloc.trace.TraceGlobalMoveResolver;
 ...
 *
 * The current implementation cannot deal with {@link BasicBlock blocks} with edges to compiled
 * exception handlers since it might introduce spill code after the {@link LIRInstruction
...
public final class BottomUpAllocator extends TraceAllocationPhase<TraceAllocationContext> {
     ...
     * {@linkplain jdk.graal.compiler.lir.alloc.trace.TraceRegisterAllocationPhase.Options#TraceRACacheStackSlots
    ...
    private void resolveFindInsertPos(BasicBlock<?> fromBlock, BasicBlock<?> toBlock) {
    ...
    private final class Allocator {
        ...
        private void allocateTrace(Trace trace) {
            try (DebugContext.Scope s = debug.scope("BottomUpAllocator", trace.getBlocks()); Indent indent = debug.logAndIndent("%s (Trace%d)", trace, trace.getId())) {
                ...
                BasicBlock<?> successorBlock = blocks[lastBlockIdx];
                // handle last block
         ...
         */
        private void resolvePhis(BasicBlock<?> from, BasicBlock<?> to) {
        ...
        private void resolveLoopBackEdge(BasicBlock<?> from, BasicBlock<?> to) {
        ...
        private void resolveIntraTraceEdge(BasicBlock<?> from, BasicBlock<?> to) {
            assert resultTraces.getTraceForBlock(from).equals(resultTraces.getTraceForBlock(to)) : "Not on the same trace? " + from + " -> " + to;
            resolveFindInsertPos(from, to);
            resolveTraceEdge(from, to);
            moveResolver.resolveAndAppendMoves();
        ...
        private void resolveTraceEdge(BasicBlock<?> from, BasicBlock<?> to) {
         ...
         */
        @SuppressWarnings("try")
        private boolean allocateBlock(BasicBlock<?> block) {
        ...
        private void processIncoming(BasicBlock<?> block, LIRInstruction instruction) {
        ...
        private void processOutgoing(BasicBlock<?> block, LIRInstruction instruction) {
